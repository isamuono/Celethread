...
//document.getElementById('new_thread-btn').addEventListener('click', openFooter());
//window.addEventListener('load', closeFooter());
...
for (i = 0; i < dropdown.length; i++) {
  dropdown[i].addEventListener("click", function() {
    ...
    var dropdownContainer = parent_element.nextElementSibling; // 隣接する兄弟要素の<div>タグを取得
...
new_thread_btn.onclick = function() {
  //$('.footer_container').css('height', '40rem');
...
function colorElement() {
  ...
  //for ( i = 0; i < color.length; i++ ) {
	  ...
	  const forEach_arr = color_arr.forEach(function(value) {
	    ...
	    } else if (value.firstElementChild.innerHTML == "#fff450") {
	      value.style.backgroundColor = "#fff450";
	    } else if (value.firstElementChild.innerHTML == "#ffd700") {
